...
public class InMemoryTaskManager implements TaskManager {
    ...
    private final HashMap<String, Boolean> validationDate = new HashMap<>(35040);
    private final List<Task> prioritizedTasks = new ArrayList<>();
    main.managers.InMemoryHistoryManager historyManager = new main.managers.InMemoryHistoryManager();
    ...
    public void removeEpic(Integer id) {
        if (epicMap.get(id) != null) {
            List<Integer> idSubFromEpic = getSubListFromEpic(id);
            historyManager.remove(id);
            if (idSubFromEpic.size() != 0) {
                idSubFromEpic.forEach(historyManager::remove);
                idSubFromEpic.forEach(Integer -> removeOldTimeWhenChangeTask(subMap.get(Integer)));
                idSubFromEpic.forEach(subMap::remove);
            }
            epicMap.remove(id);
            System.out.println("Эпик-Задача с ID-" + id + " удалена!" + "\nСуб-Задачи удалены => " + idSubFromEpic);
        } else {
            System.out.println("Отсутствует задача с указанным ID-" + id);
        }
    }
    ...
    protected boolean validateFreeTime(Task task) {
        ...
        if (task.getStartTime() != null) {
            ...
            if (!valid) {
                System.out.println("Указанное время уже занятно:");
